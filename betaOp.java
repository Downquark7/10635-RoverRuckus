...
@TeleOp
public class betaOp extends OpMode {
...
//        RevBulkData data = hub3.getBulkInputData();
        ...
        double armLengthTurningCoeff = Range.clip(0.25 * (1.2 * maxSlidePosition) / (slidePos + 0.2 * maxSlidePosition), 0.2, 0.75);
...
//            extendArmMode = false;
        ...
        if ((gamepad1.y && !gamepad1.start) || (gamepad2.y && !gamepad2.start)) {
            initArmHolding(isDepotSide ? armTarget.depotSideY : armTarget.teleOpY);
            ...
            if (!gamepad1.y && !gamepad2.y && !gamepad1.b && !gamepad2.b)
                if (angleValue > minArmSensorValue)
                    if (angleValue > armTarget.horizontal)
                        armMotor.setPower((Math.abs(spin) > 0.1 || Math.abs(right) > 0.1) ? armController5(angleValue, target, slidePos) + 0 : armController5(angleValue, target, slidePos));
