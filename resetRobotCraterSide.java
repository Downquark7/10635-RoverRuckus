...
@Autonomous
public class resetRobotCraterSide extends OpMode {
    ...
    public void init() {
        ...
        askState(states.startRaisingLift);
        askState(states.armVertical);
        askState(new state("strafe right",0,5,0));
        askState(new state("return",-15,0,0));
        askState(states.coastBackward);
        askState(states.sleep2000);
        askState(new state("strafe left",0,-7,0));
        askState(states.startLoweringLift);
        askState(states.sleep2000);
        askState(states.sleep2000);
        askState(states.sleep1000);
        askState(new state("reset lift encoder") {
            @Override
...
//        addlog(dl, "autonomous", "Done with robot.init --- waiting for start in " + this.getClass().getSimpleName());
    ...
    @Override
    public void loop() {
        Log.v("Downquark7", String.format("angleValue: %f", angle.getVoltage()));
