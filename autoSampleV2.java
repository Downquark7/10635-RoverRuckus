...
import static org.firstinspires.ftc.teamcode.robotconfig.goldLocation.RIGHT;
...
@Autonomous
public class autoSampleV2 extends OpMode {
...
//    DriveConstraints slow = new DriveConstraints(25, 40, Math.PI, Math.PI);
...
//        dashboard = FtcDashboard.getInstance();
        ...
        addState(new state("drop off marker") {
            @Override
            public void firstTime() {
                if (lastGoldLocation == RIGHT)
                    drive.followTrajectory(drive.trajectoryBuilder()
                            .turn(Math.toRadians(15))//, slow)
                            .beginComposite()
//                            .splineTo(new Pose2d(-12, -39, Math.toRadians(-65)), new SplineInterpolator(Math.toRadians(-120), Math.toRadians(-65)), slow)
                            .splineTo(new Pose2d(22, -53, Math.toRadians(-10)))//, new SplineInterpolator(Math.toRadians(-120), Math.toRadians(-10)), slow)
//                            .forward(8)
                            .closeComposite()
                            .build());
                else
                    drive.followTrajectory(drive.trajectoryBuilder()
                            ...
                            .beginComposite()
                            .splineTo(new Pose2d(-12, -39, Math.toRadians(-65)))//, new SplineInterpolator(Math.toRadians(-120), Math.toRadians(-65)), slow)
                            .splineTo(new Pose2d(12, -60, 0))//, new SplineInterpolator(Math.toRadians(-65), 0), slow)
                            .forward(8)
                            .closeComposite()
                            .build());
            }
            ...
            public boolean everyTime() {
                ...
                if (currentPose.getX() > 5)
                    aux.setArmSlowly(armTarget.horizontal, lastGoldLocation == RIGHT ? (int) (0.8 * maxSlidePosition) : slidePositions[3]);
                else
        ...
        addState(new state("line up for double sample") {
            @Override
            public void firstTime() {
                if(lastGoldLocation == RIGHT)
                    return;
                drive.followTrajectory(drive.trajectoryBuilder()
                        .reverse()
                        .lineTo(new Vector2d(12, -58))//, new ConstantInterpolator(0), slow)
                        .build());
            }
            ...
            public boolean everyTime() {
                if(lastGoldLocation == RIGHT)
                    return true;
                drive.update();
                return aux.setArmSlowly(armTarget.horizontal, (int) (maxSlidePosition * 0.52)) && !drive.isFollowingTrajectory();
            }
        });

        addState(new state("double sample turn") {
            double armTarget = 0;
            int slideTarget = 0;
            ...
            public void firstTime() {
                ...
                drive.followTrajectory(drive.trajectoryBuilder()
                        .turnTo(Math.toRadians(45))//, slow)
                        .forward(3)
                        .turn(Math.toRadians(-20))//, slow)
                        .build());

                switch (lastGoldLocation) {
                    case LEFT:
                        slideTarget = maxSlidePosition;
                        break;
                    case RIGHT:
                        slideTarget = (int) (maxSlidePosition * 0.2);
                        break;
                    default:
                        slideTarget = (int) (maxSlidePosition * 0.6);
                }
            }
            ...
            public boolean everyTime() {
                ...
                drive.update();
                return aux.setArm(armPositionFinder(slideTarget), slideTarget) && !drive.isFollowingTrajectory();
            }
        });
        ...
        addState(new state("pickup gold") {
            ...
            @Override
            public boolean everyTime() {
                aux.setArm(armPositionFinder(maxSlidePosition * 0.28), (int) (maxSlidePosition * 0.28));//change to these values to down more if picking up a cube is desired
                sendPositionalErrorTelemetry();
                drive.update();
                return !drive.isFollowingTrajectory();
            }
        ...
        for (int i = 1; i < 2; i++) {
            addState(new state("go to minerals") {
                @Override
                public void firstTime() {
                    if(lastGoldLocation != CENTER)
                        return;
                    aux.activateIntake();
                    drive.followTrajectory(drive.trajectoryBuilder().splineTo(craterPos).build());
                }
...
//                    }
        ...
        addState(new state("park at end") {
            ...
            public void firstTime() {
                aux.activateIntake();
                drive.followTrajectory(drive.trajectoryBuilder().waitFor(0.3).splineTo(new Pose2d(-28, -28, Math.toRadians(-135))).build());
            }
        ...
        addState(new state("extend out arm at end") {
            ...
            public boolean everyTime() {
                sendPositionalErrorTelemetry();
                drive.update();
                aux.setArmSlowly(armTarget.hh, (int) (maxSlidePosition * 0.7));
                return false;
            }
        });

        addState(new state("sleep") {
            public void firstTime() {
                try {
                    sleep(0);
                } catch (InterruptedException e) {
