...
@Autonomous
public class autoDepotNearV2 extends OpMode {
...
//        dashboard = FtcDashboard.getInstance();
        ...
        addState(new state("go to crater") {
            ...
            public void firstTime() {
                drive.followTrajectory(drive.trajectoryBuilder()
                        .beginComposite()
                        .turn(Math.toRadians(-25))
                        .splineTo(new Pose2d(-26, -64, Math.toRadians(-180)),new LinearInterpolator(Math.toRadians(-70),Math.toRadians(-180 + 360)))
                        .closeComposite()
                        .build());
            }
        ...
        addState(new state("extend out arm at end") {
            @Override
            public void firstTime() {
                drive.followTrajectory(drive.trajectoryBuilder().turnTo(Math.toRadians(-180)).build());
            }
            ...
            public boolean everyTime() {
                sendPositionalErrorTelemetry();
                drive.update();
                return aux.setArmSlowly(armTarget.horizontal, slidePositions[0]) && !drive.isFollowingTrajectory();
            }
        });
